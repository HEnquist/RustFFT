...
impl<T: FftNum> FftPlannerNeon<T> {
    ...
    pub fn new() -> Result<Self, ()> {
        if std::arch::is_aarch64_feature_detected!("neon") {
            ...
            if id_t == id_f32 || id_t == id_f64 {
                ...
                let prime_butterflies = neon_prime_butterflies::prime_butterfly_lens();
    ...
    fn build_new_fft(&mut self, recipe: &Recipe, direction: FftDirection) -> Arc<dyn Fft<T>> {
        ...
        match recipe {
            ...
            Recipe::Butterfly24 => {
                if id_t == id_f32 {
                    Arc::new(NeonF32Butterfly24::new(direction)) as Arc<dyn Fft<T>>
                } else if id_t == id_f64 {
                    Arc::new(NeonF64Butterfly24::new(direction)) as Arc<dyn Fft<T>>
                } else {
                    panic!("Not f32 or f64");
                }
            }
            ...
            Recipe::PrimeButterfly { len } => unsafe {
                neon_prime_butterflies::construct_prime_butterfly(*len, direction)
            },
            Recipe::MixedRadix {
                left_fft,
                right_fft,
    ...
    fn design_butterfly_algorithm(&mut self, len: usize) -> Option<Arc<Recipe>> {
        match len {
            ...
            _ => {
                if neon_prime_butterflies::prime_butterfly_lens().contains(&len) {
...
mod unit_tests {
    ...
    fn test_plan_neon_butterflies() {
        ...
        for len in neon_prime_butterflies::prime_butterfly_lens() {
            assert_eq!(
