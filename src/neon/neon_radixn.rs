//! A NEON version of `RadixN`, generic over f32 and f64.
//!
...
//! cross-FFT layers use NEON column butterflies instead of the scalar ones, so a whole vector of
...
/// The column butterflies `NeonRadixN` needs that aren't already on `NeonVector`.
///
/// Radix 2 and 4 are already vector-generic as `NeonVector::column_butterfly2` and
...
pub trait RadixNButterflies: NeonVector + Sized {
...
impl RadixNButterflies for float64x2_t {
...
impl RadixNButterflies for float32x4_t {
...
pub struct NeonRadixN<N: NeonNum, T> {
...
impl<N: NeonNum, T: FftNum> NeonRadixN<N, T> {
    /// Constructs a NeonRadixN which computes FFTs of length `factor_product * base_fft.len()`.
    pub fn new(factors: &[RadixFactor], base_fft: Arc<dyn Fft<T>>) -> Self {
        ...
        let complex_per_vector = <N::VectorType as NeonVector>::COMPLEX_PER_VECTOR;
        ...
        assert!(
            ...
            "NeonRadixN requires a base length divisible by {}, got {}",
            complex_per_vector,
            base_len
        );
        ...
        for factor in factors {
            ...
            layers.push(unsafe {
                match factor {
                    ...
                    RadixFactor::Factor4 => Layer::Factor4(NeonVector::make_rotate90(direction)),
                    RadixFactor::Factor5 => {
                ...
                match layer {
                    Layer::Factor2 => {
                        cross_layer::<N, 2, _>(data, layer_twiddles, num_columns, |v| {
                            NeonVector::column_butterfly2(v)
                        })
                    }
                    Layer::Factor3(bf) => {
                    ...
                    Layer::Factor4(rotation) => {
                        cross_layer::<N, 4, _>(data, layer_twiddles, num_columns, |v| {
                            NeonVector::column_butterfly4(v, *rotation)
                        })
                    }
                    Layer::Factor5(bf) => {
...
boilerplate_fft_neon_oop_scratch!(NeonRadixN, |this: &NeonRadixN<_, _>| this.len);
...
///
/// Unrolled two vectors at a time, which is what NeonRadix4's `butterfly_4` does and is what gets
/// the two independent dependency chains needed to keep the FMA pipeline busy.
#[inline(always)]
unsafe fn cross_layer<N: NeonNum, const RADIX: usize, F>(
...
mod unit_tests {
    ...
    /// `cargo test --release -- --ignored neon_radixn_six_layers`.
    #[test]
    #[ignore]
    ...
    fn test_radixn<T: NeonNum + Float + SampleUniform>(
        factors: &[RadixFactor],
        base_fft: Arc<dyn Fft<T>>,
    ) {
        let len = base_fft.len() * factors.iter().map(|f| f.radix()).product::<usize>();
        let direction = base_fft.fft_direction();
        let fft: NeonRadixN<T, T> = NeonRadixN::new(factors, base_fft);
