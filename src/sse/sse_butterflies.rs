...
impl<T: FftNum> SseF32Butterfly2<T> {
    ...
    // result is [X0, X1]
    #[inline(always)]
    pub(crate) unsafe fn perform_fft_direct(&self, values: SseF32x4) -> SseF32x4 {
...
// result is [X0, Y0], [X1, Y1]
#[inline(always)]
unsafe fn dual_fft2_interleaved_f32(val02: SseF32x4, val13: SseF32x4) -> [SseF32x4; 2] {
    let temp0 = val02 + val13;
    let temp1 = val02 - val13;
    [temp0, temp1]
}
...
// result is [X0, Y0], [X1, Y1]
#[inline(always)]
unsafe fn dual_fft2_contiguous_f32(left: SseF32x4, right: SseF32x4) -> [SseF32x4; 2] {
    let temp02 = pack_1st_f32(&left, &right);
    let temp13 = pack_2nd_f32(&left, &right);
    let temp0 = temp02 + temp13;
    let temp1 = temp02 - temp13;
    [temp0, temp1]
}
...
// result is [X0, X1]
#[inline(always)]
unsafe fn solo_fft2_f32(values: SseF32x4) -> SseF32x4 {
    let temp = reverse_complex_elements_f32(&values);
    let sign = _mm_set_ps(-0.0, -0.0, 0.0, 0.0);
    let temp2 = SseF32x4::new(_mm_xor_ps(values.get(), sign));
    temp2 + temp
}
...
impl<T: FftNum> SseF32Butterfly3<T> {
    ...
    pub fn new(direction: FftDirection) -> Self {
        ...
        let twiddle = SseF32x4::new(unsafe { _mm_set_ps(-tw1.im, -tw1.im, tw1.re, tw1.re) });
        let twiddle1re = SseF32x4::new(unsafe { _mm_set_ps(tw1.re, tw1.re, tw1.re, tw1.re) });
        let twiddle1im = SseF32x4::new(unsafe { _mm_set_ps(tw1.im, tw1.im, tw1.im, tw1.im) });
        Self {
            direction,
            _phantom: std::marker::PhantomData,
            rotate,
    ...
    pub(crate) unsafe fn perform_fft_direct(
        ...
        let temp12pn = self.rotate.rotate_2nd(&(value12 +rev12));
        let twiddled = temp12pn * self.twiddle;
        let temp = value0x + twiddled;
        let out12 = solo_fft2_f32(temp);
        let out0x = value0x + temp12pn;
        [out0x, out12]
    }
    ...
    pub(crate) unsafe fn perform_dual_fft_direct(
        ...
        // This is a SSE translation of the scalar 3-point butterfly
        let x12p = value1 + value2;
        let x12n = value1 - value2;
        let sum = value0 + x12p;

        let temp_a = self.twiddle1re * x12p;
        let temp_a = temp_a + value0;

        let n_rot = self.rotate.rotate_both(&x12n);
        let temp_b = self.twiddle1im * n_rot;

        let x1 = temp_a + temp_b;
        let x2 = temp_a - temp_b;
        [sum, x1, x2]
    }
}
...
pub struct SseF32Butterfly5<T> {
    ...
    _phantom: std::marker::PhantomData<T>,
    rotate: Rotate90F32,
    twiddle12re: SseF32x4,
    twiddle21re: SseF32x4,
    twiddle12im: SseF32x4,
    twiddle21im: SseF32x4,
    twiddle1re: SseF32x4,
    twiddle1im: SseF32x4,
    twiddle2re: SseF32x4,
    twiddle2im: SseF32x4,
...
impl<T: FftNum> SseF32Butterfly5<T> {
    ...
    pub fn new(direction: FftDirection) -> Self {
        ...
        let twiddle12re = SseF32x4::new(unsafe { _mm_set_ps(tw2.re, tw2.re, tw1.re, tw1.re) });
        let twiddle21re = SseF32x4::new(unsafe { _mm_set_ps(tw1.re, tw1.re, tw2.re, tw2.re) });
        let twiddle12im = SseF32x4::new(unsafe { _mm_set_ps(tw2.im, tw2.im, tw1.im, tw1.im) });
        let twiddle21im = SseF32x4::new(unsafe { _mm_set_ps(-tw1.im, -tw1.im, tw2.im, tw2.im) });
        let twiddle1re = SseF32x4::new(unsafe { _mm_set_ps(tw1.re, tw1.re, tw1.re, tw1.re) });
        let twiddle1im = SseF32x4::new(unsafe { _mm_set_ps(tw1.im, tw1.im, tw1.im, tw1.im) });
        let twiddle2re = SseF32x4::new(unsafe { _mm_set_ps(tw2.re, tw2.re, tw2.re, tw2.re) });
        let twiddle2im = SseF32x4::new(unsafe { _mm_set_ps(tw2.im, tw2.im, tw2.im, tw2.im) });
    ...
    pub(crate) unsafe fn perform_fft_direct(
        ...
        let x1423p = value12 + temp43;
        let x1423n = value12 - temp43;
        ...
        let temp_a1 = self.twiddle12re * x1414p;
        let temp_a2 = self.twiddle21re * x2323p;

        let temp_b1 = self.twiddle12im * x1414n;
        let temp_b2 = self.twiddle21im * x2323n;

        let temp_a = temp_a1 + temp_a2;
        let temp_a = value00 + temp_a;

        let temp_b = temp_b1 + temp_b2;

        let b_rot = self.rotate.rotate_both(&temp_b);

        let x00 = value00 + x1414p + x2323p;

        let x12 = temp_a + b_rot;
        let x34 = reverse_complex_elements_f32(&(temp_a - b_rot));
        [x00, x12, x34]
    }
    ...
    pub(crate) unsafe fn perform_dual_fft_direct(
        ...
        // This is a SSE translation of the scalar 3-point butterfly
        let x14p = value1 + value4;
        let x14n = value1 - value4;
        let x23p = value2 + value3;
        let x23n = value2 - value3;

        let temp_a1_1 = self.twiddle1re * x14p;
        let temp_a1_2 = self.twiddle2re * x23p;
        let temp_b1_1 = self.twiddle1im * x14n;
        let temp_b1_2 = self.twiddle2im * x23n;
        let temp_a2_1 = self.twiddle1re * x23p;
        let temp_a2_2 = self.twiddle2re * x14p;
        let temp_b2_1 = self.twiddle2im * x14n;
        let temp_b2_2 = self.twiddle1im * x23n;

        let temp_a1 = value0 + temp_a1_1 + temp_a1_2;
        let temp_b1 = temp_b1_1 + temp_b1_2;
        let temp_a2 = value0 + temp_a2_1 + temp_a2_2;
        let temp_b2 = temp_b2_1 - temp_b2_2;

        [
            value0 + x14p + x23p,
            temp_a1 + self.rotate.rotate_both(&temp_b1),
            temp_a2 + self.rotate.rotate_both(&temp_b2),
            temp_a2 - self.rotate.rotate_both(&temp_b2),
            temp_a1 - self.rotate.rotate_both(&temp_b1),
        ]
    }
...
impl<T: FftNum> SseF32Butterfly8<T> {
    ...
    pub fn new(direction: FftDirection) -> Self {
        ...
        let root2 = SseF32x4::new(unsafe { _mm_set_ps(0.5f32.sqrt(), 0.5f32.sqrt(), 1.0, 1.0) });
        let root2_dual = SseF32x4::new(unsafe { _mm_load1_ps(&0.5f32.sqrt()) });
    ...
    #[inline(always)]
    unsafe fn perform_fft_direct(&self, values: [SseF32x4; 4]) -> [SseF32x4; 4] {
        ...
        let val2c = val2b + val2[0];
        let val2d = val2c * self.root2;
        val2[0] = pack_1and2_f32(&val2[0], &val2d);

        let val3b = self.rotate90.rotate_both(&val2[1]);
        let val3c = val3b - val2[1];
        let val3d = val3c * self.root2;
    ...
    #[inline(always)]
    unsafe fn perform_dual_fft_direct(&self, values: [SseF32x4; 8]) -> [SseF32x4; 8] {
        // we're going to hardcode a step of split radix
        // step 1: copy and reorder the input into the scratch
        // and
        // step 2: column FFTs
        ...
        let val5c = val5b + val47[1];
        val47[1] = val5c * self.root2_dual;
        val47[2] = self.rotate90.rotate_both(&val47[2]);
        let val7b = self.rotate90.rotate_both(&val47[3]);
        let val7c = val7b - val47[3];
        val47[3] = val7c * self.root2_dual;
...
impl<T: FftNum> SseF32Butterfly9<T> {
    ...
    pub fn new(direction: FftDirection) -> Self {
        ...
        let twiddle1 = SseF32x4::new(unsafe { _mm_set_ps(tw1.im, tw1.re, tw1.im, tw1.re) });
        let twiddle2 = SseF32x4::new(unsafe { _mm_set_ps(tw2.im, tw2.re, tw2.im, tw2.re) });
        let twiddle4 = SseF32x4::new(unsafe { _mm_set_ps(tw4.im, tw4.re, tw4.im, tw4.re) });
        Self {
            direction,
            _phantom: std::marker::PhantomData,
            bf3,
    ...
    #[inline(always)]
    pub(crate) unsafe fn perform_dual_fft_direct(&self, values: [SseF32x4; 9]) -> [SseF32x4; 9] {
...
impl<T: FftNum> SseF32Butterfly10<T> {
    ...
    #[inline(always)]
    pub(crate) unsafe fn perform_fft_direct(&self, values: [SseF32x4; 5]) -> [SseF32x4; 5] {
    ...
    #[inline(always)]
    pub(crate) unsafe fn perform_dual_fft_direct(&self, values: [SseF32x4; 10]) -> [SseF32x4; 10] {
        // Algorithm: 5x2 good-thomas
...
impl<T: FftNum> SseF32Butterfly12<T> {
    ...
    #[inline(always)]
    pub(crate) unsafe fn perform_fft_direct(&self, values: [SseF32x4; 6]) -> [SseF32x4; 6] {
    ...
    #[inline(always)]
    pub(crate) unsafe fn perform_dual_fft_direct(&self, values: [SseF32x4; 12]) -> [SseF32x4; 12] {
        // Algorithm: 4x3 good-thomas
...
impl<T: FftNum> SseF32Butterfly15<T> {
    ...
    #[inline(always)]
    pub(crate) unsafe fn perform_dual_fft_direct(&self, values: [SseF32x4; 15]) -> [SseF32x4; 15] {
...
pub struct SseF32Butterfly16<T> {
    ...
    twiddle01: SseF32x4,
    twiddle23: SseF32x4,
    twiddle01conj: SseF32x4,
    twiddle23conj: SseF32x4,
    twiddle1: SseF32x4,
    twiddle2: SseF32x4,
    twiddle3: SseF32x4,
    twiddle1c: SseF32x4,
...
impl<T: FftNum> SseF32Butterfly16<T> {
    ...
    pub fn new(direction: FftDirection) -> Self {
        ...
        let twiddle01 = SseF32x4::new(unsafe { _mm_set_ps(tw1.im, tw1.re, 0.0, 1.0) });
        let twiddle23 = SseF32x4::new(unsafe { _mm_set_ps(tw3.im, tw3.re, tw2.im, tw2.re) });
        let twiddle01conj = SseF32x4::new(unsafe { _mm_set_ps(-tw1.im, tw1.re, 0.0, 1.0) });
        let twiddle23conj = SseF32x4::new(unsafe { _mm_set_ps(-tw3.im, tw3.re, -tw2.im, tw2.re) });
        let twiddle1 = SseF32x4::new(unsafe { _mm_set_ps(tw1.im, tw1.re, tw1.im, tw1.re) });
        let twiddle2 = SseF32x4::new(unsafe { _mm_set_ps(tw2.im, tw2.re, tw2.im, tw2.re) });
        let twiddle3 = SseF32x4::new(unsafe { _mm_set_ps(tw3.im, tw3.re, tw3.im, tw3.re) });
        let twiddle1c = SseF32x4::new(unsafe { _mm_set_ps(-tw1.im, tw1.re, -tw1.im, tw1.re) });
        let twiddle2c = SseF32x4::new(unsafe { _mm_set_ps(-tw2.im, tw2.re, -tw2.im, tw2.re) });
        let twiddle3c = SseF32x4::new(unsafe { _mm_set_ps(-tw3.im, tw3.re, -tw3.im, tw3.re) });
        Self {
            direction,
            bf4,
            bf8,
    ...
    #[inline(always)]
    unsafe fn perform_fft_direct(&self, input: [SseF32x4; 8]) -> [SseF32x4; 8] {
        // we're going to hardcode a step of split radix
        // step 1: copy and reorder the input into the scratch
    ...
    #[inline(always)]
    unsafe fn perform_dual_fft_direct(&self, input: [SseF32x4; 16]) -> [SseF32x4; 16] {
        // we're going to hardcode a step of split radix
        // step 1: copy and reorder the  input into the scratch
        // and
        // step 2: column FFTs
...
pub struct SseF32Butterfly32<T> {
    ...
    twiddle01conj: SseF32x4,
    twiddle23conj: SseF32x4,
    twiddle45conj: SseF32x4,
    twiddle67conj: SseF32x4,
    twiddle1: SseF32x4,
    twiddle2: SseF32x4,
    twiddle3: SseF32x4,
    twiddle4: SseF32x4,
...
impl<T: FftNum> SseF32Butterfly32<T> {
    ...
    pub fn new(direction: FftDirection) -> Self {
        ...
        let twiddle01 = SseF32x4::new(unsafe { _mm_set_ps(tw1.im, tw1.re, 0.0, 1.0) });
        let twiddle23 = SseF32x4::new(unsafe { _mm_set_ps(tw3.im, tw3.re, tw2.im, tw2.re) });
        let twiddle45 = SseF32x4::new(unsafe { _mm_set_ps(tw5.im, tw5.re, tw4.im, tw4.re) });
        let twiddle67 = SseF32x4::new(unsafe { _mm_set_ps(tw7.im, tw7.re, tw6.im, tw6.re) });
        let twiddle01conj = SseF32x4::new(unsafe { _mm_set_ps(-tw1.im, tw1.re, 0.0, 1.0) });
        let twiddle23conj = SseF32x4::new(unsafe { _mm_set_ps(-tw3.im, tw3.re, -tw2.im, tw2.re) });
        let twiddle45conj = SseF32x4::new(unsafe { _mm_set_ps(-tw5.im, tw5.re, -tw4.im, tw4.re) });
        let twiddle67conj = SseF32x4::new(unsafe { _mm_set_ps(-tw7.im, tw7.re, -tw6.im, tw6.re) });
        let twiddle1 = SseF32x4::new(unsafe { _mm_set_ps(tw1.im, tw1.re, tw1.im, tw1.re) });
        let twiddle2 = SseF32x4::new(unsafe { _mm_set_ps(tw2.im, tw2.re, tw2.im, tw2.re) });
        let twiddle3 = SseF32x4::new(unsafe { _mm_set_ps(tw3.im, tw3.re, tw3.im, tw3.re) });
        let twiddle4 = SseF32x4::new(unsafe { _mm_set_ps(tw4.im, tw4.re, tw4.im, tw4.re) });
        let twiddle5 = SseF32x4::new(unsafe { _mm_set_ps(tw5.im, tw5.re, tw5.im, tw5.re) });
        let twiddle6 = SseF32x4::new(unsafe { _mm_set_ps(tw6.im, tw6.re, tw6.im, tw6.re) });
        let twiddle7 = SseF32x4::new(unsafe { _mm_set_ps(tw7.im, tw7.re, tw7.im, tw7.re) });
        let twiddle1c = SseF32x4::new(unsafe { _mm_set_ps(-tw1.im, tw1.re, -tw1.im, tw1.re) });
        let twiddle2c = SseF32x4::new(unsafe { _mm_set_ps(-tw2.im, tw2.re, -tw2.im, tw2.re) });
        let twiddle3c = SseF32x4::new(unsafe { _mm_set_ps(-tw3.im, tw3.re, -tw3.im, tw3.re) });
        let twiddle4c = SseF32x4::new(unsafe { _mm_set_ps(-tw4.im, tw4.re, -tw4.im, tw4.re) });
        let twiddle5c = SseF32x4::new(unsafe { _mm_set_ps(-tw5.im, tw5.re, -tw5.im, tw5.re) });
        let twiddle6c = SseF32x4::new(unsafe { _mm_set_ps(-tw6.im, tw6.re, -tw6.im, tw6.re) });
        let twiddle7c = SseF32x4::new(unsafe { _mm_set_ps(-tw7.im, tw7.re, -tw7.im, tw7.re) });
        Self {
            direction,
            bf8,
            bf16,
    ...
    #[inline(always)]
    unsafe fn perform_fft_direct(&self, input: [SseF32x4; 16]) -> [SseF32x4; 16] {
        // we're going to hardcode a step of split radix
    ...
    #[inline(always)]
    pub(crate) unsafe fn perform_dual_fft_direct(&self, input: [SseF32x4; 32]) -> [SseF32x4; 32] {
        // we're going to hardcode a step of split radix
...
mod unit_tests {
    ...
    fn test_dual_fft4_32() {
        unsafe {
            ...
            let res = std::mem::transmute::<[SseF32x4; 4], [Complex<f32>; 8]>(res_both);
